...
// 但同事若一直泡在老会话里不重启，这个会话就一直跑旧版——新规则、新 hook
...
//   之后的版本更新」起作用——越早铺开越省心。
// 旁路：PCP_VERSION_REMINDER=off 关闭。
// =============================================================
...
// —— 每个插件复制本文件时，只改这三行 ——
const MARKETPLACE = 'project-coding-profiles'; // 本插件所在 marketplace 名
const PLUGIN = 'project-coding-profiles';      // 本插件名（marketplace.json plugins[].name）
const OFF_ENV = 'PCP_VERSION_REMINDER';        // 关闭开关环境变量名
...
process.stdin.on('end', () => {
  try {
    ...
    process.stderr.write(
      ...
      `  新规则 / 新 hook 不会在本会话生效——请重启 Claude Code 会话（开新会话）后再继续。\n` +
